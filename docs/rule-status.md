...
| [`import/no-cycle`](https://github.com/import-js/eslint-plugin-import/blob/main/docs/rules/no-cycle.md) | Implemented for relative imports and re-exports resolved with fishlint's configured extensions, with `maxDepth` (integer, `"∞"`, or `Infinity` through the JavaScript API), `commonjs`, and `amd` configuration |
...
| [`react/prop-types`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/prop-types.md) | Supports `skipUndeclared`, `ignore`, and `customValidators` configuration ; recognizes React `forwardRef<Ref, Props>` generics through `memo` wrappers |
...
| [`react-hooks/void-use-memo`](https://github.com/facebook/react/blob/e92bda78750136493cb324e98df1726f62ba8e92/compiler/packages/babel-plugin-react-compiler/src/Validation/ValidateUseMemo.ts) | Opt-in; checks inline and statically resolved named callbacks with no value return, excluding returns in nested functions. Also checks discarded expression-statement results. Explicit returns (including bare returns), expression bodies, and throwing callbacks are accepted; no all-path return or inferred return-type analysis |
| [`react-hooks/purity`](https://react.dev/reference/eslint-plugin-react-hooks/lints/purity) | Opt-in; checks global `Math.random`, `Date.now`, `Date()`, zero-argument `new Date`, `crypto.randomUUID`, and `performance.now` and their stable aliases during render, inline/named `useMemo`, and synchronous IIFEs. Skips deferred callbacks, lazy initializers, and shadowed globals |
| [`react-hooks/set-state-in-render`](https://react.dev/reference/eslint-plugin-react-hooks/lints/set-state-in-render) | Opt-in; checks unconditionally executed calls to `useState` setters and stable aliases, including inline/named `useMemo` and synchronous IIFEs. Covers setters after normally completing loops and branches; skips conditional updates, possible preceding early exits, reassigned setters, and deferred callbacks |
